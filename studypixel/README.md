...
## 🛠️ Latest Updates (Automated Upgrade)

- P0 Security: Rotated secrets recommended, added `functions/.env*` to `.gitignore`, added sanitization helpers to Cloud Functions to mitigate prompt injection, and restricted `seedWidgets` to admin-only.
- P1 Quality: Removed duplicate dashboard folder, removed CommonJS dead-exports from hooks, fixed duplicate object properties and stray throws, and gated demo credentials to development only.
- CI: Added a GitHub Actions CI workflow for linting and type checking.
- Build: Removed `ignoreBuildErrors` and enabled type checking; added TypeScript config for `functions`.

These changes were applied automatically to improve security and maintainability. Please rotate any exposed API keys and re-run `npm install` and `npm run build` locally to validate.
