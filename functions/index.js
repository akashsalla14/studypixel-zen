...
// Lightweight logging shim used across this module. Keeps telemetry-compatible signature.
function logAndCapture(level, message, meta) {
 ...
 * @param {number} maxRetries Number of retry attempts if JSON parsing fails.
 * @return {Promise<object>} A structured evaluation response.
 */
async function _sendToGemini(payload, maxRetries = 2) {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      lastRawText = rawResponseText;

      const parsedResponse = parseLlmJson(rawResponseText);
      if (parsedResponse) {
      ...
      logger.warn(`JSON parsing failed for Gemini (${bestModelId}) [Attempt ${attempt}/${maxRetries}]. Retrying...`, {text: rawResponseText});
    } catch (error) {
      logger.error(`Failed to call Gemini API [Attempt ${attempt}/${maxRetries}]:`, error);
  ...
  logger.warn(`Exhausted retries for Gemini. Falling back to plain text.`);
  const fallbackResponse = {action: "SPEAK", mentor_speech: lastRawText};
  const confidence = calculateConfidence(fallbackResponse, bestModelId);
  return {model: `Gemini (${bestModelId})`, response: fallbackResponse, confidence};
}
 ...
 * @return {Promise<object>} A structured evaluation response.
 */
async function _sendToOpenAI(payload, maxRetries = 2) {
  try {
    let apiKey;
    try {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (!response.ok) {
          ...
          logger.warn(`OpenAI API Error [Attempt ${attempt}/${maxRetries}]:`, {status: response.status, body: errorBody});
        ...
        const parsedResponse = parseLlmJson(rawResponseText);
        if (parsedResponse) {
          const confidence = calculateConfidence(parsedResponse, "gpt-4o-mini");
          return {model: "ChatGPT", response: parsedResponse, confidence};
        }
        logger.warn(`JSON parsing failed for OpenAI [Attempt ${attempt}/${maxRetries}]. Retrying...`, {text: rawResponseText});
      } catch (error) {
        if (attempt === maxRetries) throw error;
      }
    }

    logger.warn(`Exhausted retries for OpenAI. Falling back to plain text.`);
    const fallbackResponse = {action: "SPEAK", mentor_speech: lastRawText};
    const confidence = calculateConfidence(fallbackResponse, "gpt-4o-mini");
    return {model: "ChatGPT", response: fallbackResponse, confidence};
  } catch (error) {
 ...
 * @return {Promise<object>} A structured evaluation response.
 */
async function sendToDigitalOcean(payload, memberConfig, maxRetries = 2) {
  try {
    let apiKey;
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (!response.ok) {
          const errorBody = await response.text();
          logger.warn(`DigitalOcean API Error (${memberConfig.role}) [Attempt ${attempt}/${maxRetries}]:`, {status: response.status, body: errorBody});
        ...
        logger.warn(`JSON parsing failed for DO (${memberConfig.role}) [Attempt ${attempt}/${maxRetries}]. Retrying...`, {text: rawResponseText});
      } catch (error) {
        if (attempt === maxRetries) throw error;
      }
    }

    logger.warn(`Exhausted retries for DO (${memberConfig.role}). Falling back to SPEAK action.`);
    const fallbackResponse = {action: "SPEAK", mentor_speech: lastRawText};
    const confidence = calculateConfidence(fallbackResponse, memberConfig.modelId);
    return {model: `DO-${memberConfig.role}`, response: fallbackResponse, confidence};
  } catch (error) {
 ...
 */
async function _generateImageWithDO(prompt, logLogger) {
  try {
    let apiKey;
    if (typeof digitalOceanApiKey !== "undefined" && digitalOceanApiKey) {
    ...
    if (!apiKey) {
      ...
      logger.warn(msg);
      if (logLogger) logLogger("WARN", msg);
    ...
    for (const {id, size} of tryModels) {
      try {
        ...
        if (!response.ok) {
          ...
          if (logLogger) logLogger("WARN", warnMsg, {status: response.status});
    ...
    if (logLogger) logLogger("ERROR", "Image Generation Failed", {message: error.message});
...
/**
 * Callable wrapper to run student code without performing BKT/LLM council updates.
 * Returns sanitized execution result for UI preview.
 */
exports.runSandboxCode = onCall({region: "us-east1", memory: "512MiB", cors: true}, async (request) => {
  ...
  const validationTest = String(payload.validationTest || "");
  const widgetId = String(payload.widgetId || "");
  const attemptId = String(payload.attemptId || "");

  const allowedLangs = ["python", "javascript", "bash", "java"];
  if (!widgetId) throw new HttpsError("invalid-argument", "Missing widgetId.");
  if (!allowedLangs.includes(language)) throw new HttpsError("invalid-argument", `Unsupported language: ${language}`);
  const CODE_MAX = parseInt(process.env.SANDBOX_CODE_MAX || "20000", 10);
  if (code.length > CODE_MAX) throw new HttpsError("invalid-argument", `Code exceeds maximum length of ${CODE_MAX} characters.`);
  if (validationTest.length > 2000) throw new HttpsError("invalid-argument", "validationTest too long.");

  // Rate limit per user for sandbox runs
  try {
    await checkRateLimit(userId, "sandbox-run", 20, 60);
  } catch (err) {
    throw new HttpsError("resource-exhausted", "Rate limit exceeded for sandbox runs.");
  }

  const start = Date.now();
  const execResult = await executeCodeWithJudge0(language, code, validationTest);
...
// Authoritative submission: executes tests, persists server-validated telemetry, and updates BKT deterministically
exports.submitSandboxSolution = onCall({region: "us-east1", memory: "512MiB", cors: true}, async (request) => {
  if (!request.auth) throw new HttpsError("unauthenticated", "Must be authenticated.");
  const userId = request.auth.uid;
  const payload = request.data || {};
  const language = String(payload.language || "python").toLowerCase();
  ...
  const validationTest = String(payload.validationTest || "");
  const widgetId = String(payload.widgetId || "");
  const attemptId = String(payload.attemptId || (Date.now() + "-" + Math.random().toString(36).slice(2, 9)));
  const pixelBotId = String(payload.pixelBotId || payload.pixelBotId === 0 ? payload.pixelBotId : (payload.pixelBotId || "unknown"));
  const topic = String(payload.topic || payload.topic === 0 ? payload.topic : (payload.topic || "general"));
  ...
  // Rate limit authoritative submissions (fewer allowed than dry runs)
  try {
    await checkRateLimit(userId, "sandbox-submit", 5, 60); // e.g., 5 submits per minute
  } catch (err) {
    throw new HttpsError("resource-exhausted", "Rate limit exceeded for sandbox submissions.");
  }
  ...
  const passed = !!execResult.passed;

  // Persist server-validated telemetry to Firestore
  const db = getFirestore();
  ...
  const telemetryDoc = {
    ...
    stderr: execResult.stderr || "",
    executionMs,
    codeSnippet: code.length > 2000 ? code.slice(0, 2000) : code,
    metrics: {language, validationTest},
    savedAt: new Date().toISOString()
  };

  const addedRef = await db.collection("studentProgress").doc(storageKey).collection("telemetry").add(telemetryDoc);

  // Deterministic BKT update (no LLM involvement)
  try {
    ...
    let adaptiveProfile = {};
    if (profileSnap.exists) adaptiveProfile = profileSnap.data();

    const prior = adaptiveProfile.topicMastery?.[topic]?.mastery ?? adaptiveProfile.bktMastery ?? 0.25;
    const evidence = passed ? 1.0 : 0.0;
    const newMastery = calculatePosteriorMastery(prior, evidence);
    const nowIso = new Date().toISOString();
    const updateData = {bktMastery: newMastery, bktLastUpdated: nowIso, topicMastery: {[topic]: {mastery: newMastery, lastUpdated: nowIso}}};
    await profileRef.set(updateData, {merge: true});

    logAndCapture("INFO", "BKT mastery updated from authoritative sandbox submission", {userId, pixelBotId, topic, prior, evidence, newMastery});
  } catch (bktErr) {
    logger.warn("Failed to update BKT from sandbox submission:", bktErr);
  }
  ...
  return {
    ...
    stdout: execResult.stdout || "",
    stderr: execResult.stderr || "",
    savedTelemetryId: addedRef.id,
    attemptId
  };
});

// Retrieve recent server-validated sandbox attempts for this user+widget
exports.getSandboxAttempts = onCall({region: "us-east1", memory: "256MiB", cors: true}, async (request) => {
  if (!request.auth) throw new HttpsError("unauthenticated", "Must be authenticated.");
  const userId = request.auth.uid;
  const payload = request.data || {};
  const widgetId = String(payload.widgetId || "");
  ...
  const db = getFirestore();
  const storageKey = `progress_${userId}_${pixelBotId}`;
  const snap = await db.collection("studentProgress").doc(storageKey).collection("telemetry").where("widgetId", "==", widgetId).orderBy("savedAt", "desc").limit(20).get();
...
async function classifyIntent(payload, isAdmin = false) {
  ...
  const classifierConfig = {
    role: "Router",
    modelId: "llama-4-maverick", // Fast, cheap model
...
"""`;
  ...
  const councilMembers = [
    {
      role: "Evaluator_A",
      modelId: "gemma-4-31B-it", // Upgraded for high-quality, strict evaluation
  ...
  // Run sequentially to prevent bursting the DigitalOcean Serverless API
  // which often causes 429 Too Many Requests or 503 Service Unavailable
  const results = [];
  for (const member of councilMembers) {
    results.push(await sendToDigitalOcean(focusedPayload, member));
  }
  return results;
}
...
async function runTeachingAgent(payload, widgetList, assessmentResult, adaptiveProfile = {}, reviewTopics = []) {
  ...
  // Define widget keywords globally for strategy overrides
  ...
  const isNodeExplanationRequest = promptLower.includes("part of this diagram") || promptLower.includes("explain the");
  const isWidgetRequest = widgetKeywords.some((kw) => promptLower.includes(kw)) && !isNodeExplanationRequest;

  if (intent === "CONVERSATIONAL") {
    _strategy = "CONVERSATIONAL";
    ...
    strategyInstruction = `
      ...
      2. ${forcedAction === "USE_WIDGET" ? "Provide the requested visual, example, or widget." : `Transition to the NEXT logical concept or step in ${topic}.`}
      3. ${forcedAction === "SPEAK" ? "DO NOT repeat the previous widget or explanation. Move the lesson forward." : ""}
      4. Ask if they are ready to continue.
    `;
  } else if (intent === "META") {
    // [NEW] Lean Triage Logic for META intent
    const confusionKeywords = ["don't know", "confused", "stuck", "help", "teach me", "lost", "unsure", "explain"];
    const isDeepConfusion = confusionKeywords.some((kw) => promptLower.includes(kw)) && !isNodeExplanationRequest;

    if ((isDeepConfusion || isWidgetRequest) && hasWidgets) {
      _strategy = "ADAPTIVE_INTERVENTION";
    ...
    strategyInstruction = `
      ...
      1. ANALYZE the [LONGITUDINAL STUDENT PROFILE]. Select a widget that matches their 'Learning Style' (e.g., Visual -> Diagram Generator for structured visuals, Code-first -> Sandbox).
    ...
    strategyInstruction = `
      ...
      1. Praise briefly.
      2. ${forcedAction === "USE_WIDGET" ? "Provide the requested visual, example, or widget." : "Introduce the next logical concept or increase difficulty."}
      3. ${forcedAction === "SPEAK" ? "Do NOT use a widget yet. Just explain/introduce the next step." : ""}
    `;
  }

  // Create a string representation of available widgets
  const visualCostPolicy = hasWidgets ? `
  VISUAL COST POLICY:
  Prefer "diagram-generator-v1" as the default low-cost visual tool for concepts, processes, comparisons, systems, timelines, flows, memory maps, architecture, relationships, cause/effect, pros/cons, and summaries.
  Use image generation only when the student explicitly needs realistic visual appearance, artwork, a physical scene, a character/object/background, anatomy-like appearance, a map-like image, or a photo/illustration that SVG shapes cannot represent.
  If using "diagram-generator-v1", choose the most suitable type from flowchart, tree, concept-map, mind-map, cycle, timeline, venn, matrix, pyramid, stack, radial, or comparison. Include theme when helpful: learning, science, cyber, warning, history, or neutral.
    ...
    "\nAVAILABLE WIDGETS (Use 'action': 'USE_WIDGET' and 'widgetId'):\n" +
      widgetList.map((w) => {
        ...
        if (w.widgetId === "image-analysis-v1") {
          desc += "\n  NOTE: Only generate an image when realistic appearance, artwork, or a physical scene is required. For structured learning visuals, use diagram-generator-v1 instead.";
        }
        return desc;
      }).join("\n") :
    "";
  ...
  const instructorConfig = {
    role: "Instructor",
    modelId: "gemma-4-31B-it", // Upgraded for best-in-class teaching and generation
...
function validateTeachingOutput(response, widgetList, currentIntent = "ANSWER") {
  ...
  // [NEW] LLM Rebellion Auto-Correction
  // If the LLM was forced to SPEAK but embedded a JSON string inside mentor_speech
  if (response.action === "SPEAK" && typeof response.mentor_speech === "string" && response.mentor_speech.includes("\"action\"") && response.mentor_speech.includes("\"USE_WIDGET\"")) {
    const embeddedJson = parseLlmJson(response.mentor_speech);
    if (embeddedJson && embeddedJson.action === "USE_WIDGET" && embeddedJson.widgetId) {
  ...
  if (response.action && typeof response.action === "string") {
    ...
    if (response.action === "USE_WIDGETS") response.action = "USE_WIDGET"; // Common plural mistake
  }
  ...
  if (response.action === "USE_WIDGET") {
    ...
    const validWidget = widgetList.find(
        (w) => w.id.toLowerCase() === targetId
    );
...
exports.evaluateWithCouncil = onCall({region: "us-east1", memory: "512MiB", cors: true}, async (request) => {
 ...
 parsed = parseLlmJson(jsonStr);
      ...
      if (parsed) {
        ...
        const subjectiveWidgets = ["flashcard-v1", "spaced-review-v1", "diagram-generator-v1"];
        ...
        if (interactionSource !== "REAL_STUDENT") {
          ...
          if (widgetId === "tactical-sandbox-v1") {
             ...
             // Rate limit sandbox executions per user
             try {
               await checkRateLimit(request.auth.uid, "sandbox-run", 20, 60);
             } catch (rlErr) {
               logAndCapture("WARN", "Sandbox run rate limit exceeded", {uid: request.auth.uid});
               throw new HttpsError("resource-exhausted", "Rate limit exceeded for sandbox runs.");
             }

             const validationPayload = parsed.telemetry?.metrics?.validationTest || parsed.telemetry?.metrics?.validationMode || parsed.validationTest || "";

             const execResult = await executeCodeWithJudge0(
                 String(parsed.telemetry?.metrics?.language || "python"),
                 String(parsed.code || ""),
             ...
             if (matchingEval.canEvaluate) {
               ...
               adaptiveProfile.bktMastery = newMastery;
               adaptiveProfile.extremeHesitationCount = currentExtremeHesitationCount;
               logAndCapture("INFO", "BKT mastery updated from server-scored matching widget", {topic, prior, evidence, newMastery, matchingEval});
  ...
  if (!assessmentConsensus) {
    ...
    if (forceAnswerIntent) {
      ...
      logAndCapture("INFO", "Intent Classification (Forced by Widget)", intentResult);
    } else {
...
exports.generatePixelBotPrompt = onCall({region: "us-east1", memory: "512MiB", cors: true}, async (request) => {
  ...
  if (!request.auth) {
    throw new HttpsError(
        "unauthenticated",
        "You must be authenticated to create a PixelBot.",
    );
  }
  ...
  if (!subject || !difficulty || !style || !strictness || !challengeFrequency) {
    throw new HttpsError(
        "invalid-argument",
        "Missing required fields: subject, difficulty, style, strictness, challengeFrequency.",
    );
  }
  ...
  try {
    ...
    if (role === "student") {
      ...
      if (!teacherId || !year) {
        throw new HttpsError(
            "invalid-argument",
            "New students must have a 'teacherId' and a 'year' assigned.",
        );
      }
...
exports.seedWidgets = onCall({region: "us-east1", memory: "512MiB", cors: true}, async (request) => {
  ...
  if (!request.auth) {
    throw new HttpsError(
        "unauthenticated",
        "Must be authenticated to seed widgets"
    );
  }
  ...
  const widgets = [
    {
      ...
      validation_rules: {minOptions: 2, maxOptions: 8, mustHaveCorrectAnswer: true, promptMaxLength: 500, optionMaxLength: 200},
    ...
    },
    {
      ...
      version: "1.0",
      required_data_format: {prompt: "string", options: "{ [key: string]: string }", correctAnswer: "string", requiresReasoning: "boolean (must be true)", reasoningPrompt: "string (optional)"},
      validation_rules: {minOptions: 2, maxOptions: 6, mustHaveCorrectAnswer: true, requiresReasoningField: true, reasoningMinLength: 10},
    ...
    },
    {
      ...
      difficulty: "beginner",
      version: "1.0",
      required_data_format: {front: "string (question or prompt)", back: "string (answer or definition)"},
      validation_rules: {frontMaxLength: 200, backMaxLength: 500, mustHaveFront: true, mustHaveBack: true},
    ...
    },
    {
      ...
      difficulty: "intermediate",
      version: "1.0",
      required_data_format: {type: "string (flowchart|triangle|rectangle|tree|concept-map|mind-map|cycle|timeline|venn|matrix|pyramid|stack|radial|comparison)", prompt: "string", theme: "string (learning|science|cyber|warning|history|neutral)", labels: "[string] (simple mode)", sections: "[{title?: string, label?: string, text?: string}] (section/card mode)", nodes: "[{id: string, text: string, type?: string, x?: number, y?: number, group?: string}] (advanced mode)", edges: "[{from: string, to: string, label?: string, type?: string}] (advanced mode)", layout: "string (auto|manual|horizontal|vertical|radial|compact|layered)"},
      validation_rules: {typeAllowed: ["flowchart", "triangle", "rectangle", "tree", "concept-map", "mind-map", "cycle", "timeline", "venn", "matrix", "pyramid", "stack", "radial", "comparison"], requiresPrompt: true, requiresLabelsOrNodes: true},
    ...
    },
    {
      ...
      difficulty: "intermediate",
      version: "1.0",
      required_data_format: {prompt: "string (what to compare for)", signalA: "string (code/log/text block A)", signalB: "string (code/log/text block B)"},
      validation_rules: {promptMaxLength: 200, signalMaxLength: 1000, mustHaveBothSignals: true},
    ...
    },
    {
      ...
      difficulty: "advanced",
      version: "1.0",
      required_data_format: {taskPrompt: "string (coding challenge description)", language: "string (python|javascript|bash|java)", initialCode: "string (starter template)", validationTest: "string (test case or assertion)"},
      validation_rules: {languageAllowed: ["python", "javascript", "bash", "java"], taskPromptMaxLength: 500, initialCodeMaxLength: 2000, requiresBackendIntegration: true},
      tags: ["coding", "practice", "hands-on", "advanced"],
      learningOutcomes: ["Application", "Analysis"],
      estimatedTime: "300+ seconds",
      backendDependency: "Code Execution Service",
    ...
    },
    {
      ...
      difficulty: "beginner",
      version: "1.0",
      required_data_format: {prompt: "string", sentence: "string", correctAnswers: "[[string]]", hint: "string"},
      validation_rules: {sentenceMaxLength: 300, blankCount: "1-5", correctAnswersRequired: true, caseInsensitiveMatch: true},
    ...
    },
    {
      ...
      difficulty: "intermediate",
      version: "1.0",
      required_data_format: {prompt: "string", pairs: "[{term: string, definition: string}]"},
      validation_rules: {pairCountMin: 3, pairCountMax: 8, termMaxLength: 100, definitionMaxLength: 200, allPairsMustBeUnique: true},
    ...
    },
    {
      ...
      difficulty: "intermediate",
      version: "1.0",
      required_data_format: {prompt: "string", events: "[string]", correctOrder: "[number]"},
      validation_rules: {eventCountMin: 3, eventCountMax: 7, eventTextMaxLength: 100, correctOrderMustBeArray: true, lengthMustMatchEventsLength: true},
    ...
    },
    {
      ...
      difficulty: "intermediate",
      version: "1.0",
      required_data_format: {topics: "[{name: string, masteryScore: number, daysSinceReview: number, retentionPct: number, nextDueDate: string}]"},
      validation_rules: {masteryScoreRange: [0, 1], retentionPctRange: [0, 100], topicsMinCount: 1, topicsMaxCount: 10, requiresBKTBackend: true},
      tags: ["review", "spaced-repetition", "adaptive", "bkt"],
      learningOutcomes: ["Retention", "Long-term Memory"],
      estimatedTime: "varies",
      backendDependency: "BKT Engine",
      status: "requires-integration"
    },
    {
      widgetId: "analogy-v1",
      ...
      difficulty: "advanced",
      version: "1.0",
      required_data_format: {prompt: "string", termA: "string", termB: "string", termC: "string", correctAnswer: "string", acceptableAnswers: "[string]", explanation: "string", hint: "string"},
      validation_rules: {allTermsRequired: true, termMaxLength: 100, correctAnswerRequired: true, acceptableAnswersMinCount: 1, caseInsensitiveMatch: true},
  ...
  widgets.forEach((w) => batch.set(db.collection("widgets").doc(w.widgetId), {...w, createdAt: new Date()}));

  // 1. Meta-triggers config
  const metaTriggersRef = db.collection("config").doc("metaTriggers");
  batch.set(metaTriggersRef, {
    triggers: [
      "mcq", "multiple choice", "question", "quiz", "test", "assessment", "check knowledge", "knowledge check",
      "reasoning", "explain why", "reasoning prompt", "why", "justify", "explain", "think through",
      "memorize", "flashcard", "recall", "remember", "memory", "definition", "define", "term",
      "diagram", "visual", "flowchart", "architecture", "chart", "graph", "tree", "structure",
      "compare", "signal", "difference", "spot difference", "analyze", "security vulnerability",
      "code", "sandbox", "coding", "programming", "write code", "practice", "exercise", "challenge",
      "blank", "fill", "blank fill", "vocabulary", "word", "completion",
      "match", "pair", "matching", "terminology",
      "timeline", "sequence", "order", "chronological", "steps", "process", "flow",
      "review", "spaced", "repeat", "queue", "what to study", "study next", "retention",
      "analogy", "relationship", "relationship analogy", "is to as"
    ],
    version: "1.0",
  ...
  batch.set(validationRef, {
    ...
    rules: {
      requiredFields: {
        ...
        "diagram-generator-v1": ["type", "prompt"],
  ...
  return {
        message: `Successfully seeded ${widgets.length} widgets and 3 config documents.`,
