...
const WIDGETS = [
  ...
  // ============= PHASE 1: VISUALIZATION & ANALYSIS =============
  {
    ...
    version: "1.0",
    required_data_format: {
      type: "string (flowchart|triangle|rectangle|tree|concept-map|mind-map|cycle|timeline|venn|matrix|pyramid|stack|radial|comparison)",
      prompt: "string",
      theme: "string (learning|science|cyber|warning|history|neutral)",
      labels: "[string] (simple mode)",
      sections: "[{title?: string, label?: string, text?: string}] (section/card mode)",
      nodes: "[{id: string, text: string, type?: string, x?: number, y?: number, group?: string}] (advanced mode)",
      edges: "[{from: string, to: string, label?: string, type?: string}] (advanced mode)",
      layout: "string (auto|manual|horizontal|vertical|radial|compact|layered)"
    },
    validation_rules: {
      typeAllowed: ["flowchart", "triangle", "rectangle", "tree", "concept-map", "mind-map", "cycle", "timeline", "venn", "matrix", "pyramid", "stack", "radial", "comparison"],
      requiresPrompt: true,
      requiresLabelsOrNodes: true
    },
