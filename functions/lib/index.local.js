 ...
 */
async function sendToOllama(memberConfig, userPrompt, chatHistory = []) {
    ...
    const body = {
        ...
        max_tokens: memberConfig.maxTokens ?? 512,
        stream: false,
    };
    try {
        ...
        if (!response.ok) {
            ...
            logger.warn(`Ollama error (${memberConfig.role}):`, { status: response.status, body: errText });
        ...
        if (!parsed)
            parsed = { action: "SPEAK", mentor_speech: rawText };
        return { model: `Ollama-${memberConfig.role}`, response: parsed, confidence: 0.8 };
    }
    catch (err) {
        logger.error(`sendToOllama failed (${memberConfig.role}):`, err);
        return {
...
async function runTeachingAgent(payload, widgetList, assessmentResult, adaptiveProfile = {}, reviewTopics = []) {
    ...
    if (intent === "CONVERSATIONAL") {
        ...
        const isDeep = confusion.some((kw) => promptLower.includes(kw)) && !isNodeExplanationRequest;
        if ((isDeep || isWidgetRequest) && hasWidgets) {
            forcedAction = "USE_WIDGET";
            strategyInstruction = `STRATEGY: ADAPTIVE_INTERVENTION. User is confused. Select a widget to scaffold understanding.`;
        }
        else {
            forcedAction = "SPEAK";
    ...
    `;
        ...
        forcedAction = "SPEAK";
        strategyInstruction = `STRATEGY: PROGRESS (strong mastery). Praise briefly and introduce the next concept.`;
    }
    const availableWidgets = hasWidgets ?
        "\nAVAILABLE WIDGETS:\n" +
            widgetList.map((w) => `- ID: "${w.widgetId}"\n  Desc: ${w.description}\n  Data Schema: ${w.required_data_format}`).join("\n") :
        "";
...
function validateTeachingOutput(response, widgetList, currentIntent = "ANSWER") {
    ...
    if (response.action === "USE_WIDGET") {
        ...
        if (!valid)
            return { action: "SPEAK", mentor_speech: response.mentor_speech };
